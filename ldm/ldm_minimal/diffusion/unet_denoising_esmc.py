...
class ResBlock(nn.Module):
    ...
    def forward(self, x, temb, cemb, yemb):
        ...
        h += self.cond_proj(cemb)[:, :, None]
        h += self.label_proj(yemb)[:, :, None]
        h = self.block2(h)
        h = h + self.shortcut(x)
        return self.attn(h)
...
class UNet(nn.Module):
    def __init__(self, T, ch, ch_mult, num_res_blocks, dropout, cond_dim=14, num_classes=2):
        super().__init__()
        tdim = ch * 4
        ...
        self.head = nn.Conv1d(1, ch, kernel_size=3, padding=1)
        ...
        self.tail = nn.Sequential(
            ...
            Swish(),
            nn.Conv1d(now_ch, 1, kernel_size=3, padding=1)
        )
